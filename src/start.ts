...
const RELAY_PORT = parseInt(process.env.PORT ?? '9800', 10);
...
async function ensureCursorWithCdp(): Promise<void> {
  ...
  child.on('error', (err) => {
    ...
    console.log(`  ${C.dim}Or set CURSOR_EXE to the correct path.${C.reset}\n`);
    process.exit(1);
  });
  child.unref();
...
async function main(): Promise<void> {
  ...
  process.env.BIND_ADDR = bind.addr;
  ...
  if (!process.env.PASSWORD_FROM_ENV) {
    info('Random password — set PASSWORD env var to use a fixed one');
  }
