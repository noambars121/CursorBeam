...
// Load .env from repo root so config like V2_PROJECTS_ROOT applies whether the
...
const PORT = parseInt(process.env.V2_PORT ?? '9800', 10);
const CDP_BASE = process.env.V2_CDP_BASE ?? 'http://127.0.0.1:9222';
const POLL_MS = parseInt(process.env.V2_POLL_MS ?? '1500', 10);
const BIND_ADDR = process.env._V2_BIND_ADDR ?? '127.0.0.1';
...
async function start(): Promise<void> {
  if (!LAUNCHED_BY_START) {
    ...
    console.log('');
    const pw = process.env.V2_PASSWORD ?? '(generated at runtime)';
