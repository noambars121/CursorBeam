 ...
 * of V2_PROJECTS_ROOT, detects the current project from Cursor's window title,
 * and launches Cursor against a selected folder.
 *
 * Project source: direct child directories of V2_PROJECTS_ROOT (defaults to
...
/**
 * The scan root. `V2_PROJECTS_ROOT` env var wins; otherwise fall back to the
 * user's Music folder. This is intentionally the only source of projects.
 */
function getProjectsRoot(): string {
  const envRoot = process.env.V2_PROJECTS_ROOT;
